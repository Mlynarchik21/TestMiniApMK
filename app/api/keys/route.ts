...
export async function POST(req: Request) {
  try {
    ...
    // ✅ под твою реальную модель (apiKey + secretEnc обязательны)
    const dataEncrypted = {
      exchange: body.exchange,
      label,
      apiKey: body.apiKey, // raw apiKey хранить можно (ты так решил в схеме)
    ...
    const row = existing
      ...
      : await prisma.userKey.create({
          data: {
            ...dataEncrypted,
            user: { connect: { id: user.id } }, // ✅ через relation connect (не userId)
          },
          select: {
            id: true,
            exchange: true,
